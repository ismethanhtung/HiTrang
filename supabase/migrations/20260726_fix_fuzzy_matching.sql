-- 1. Cập nhật hàm calculate_score với thuật toán so khớp mờ LIKE và sửa lỗi đè biến v_i
...
DECLARE
  ...
  v_tf_matches int;
  v_i int;
  v_j int; -- Biến chạy riêng biệt cho vòng lặp xét 4 ý đúng/sai
  v_raw_score numeric;
...
BEGIN
  ...
  FOR v_i IN 0..(v_total_questions - 1) LOOP
    ...
    IF (v_config_type = 'SECTION_BASED' OR v_config_type = 'THPT_QG') AND v_section_id IS NOT NULL AND v_q_score > 0 THEN
      -- Đếm tổng số câu thuộc phần này trong đề thi (so khớp mờ LIKE đảo thứ tự ưu tiên III -> II -> I)
      SELECT COUNT(*) INTO v_questions_in_section 
      FROM jsonb_array_elements(v_questions) AS q
      WHERE (
        (trim(lower(v_section_id)) LIKE '%phần iii%' OR trim(lower(v_section_id)) LIKE '%phần 3%')
        AND (trim(lower(q ->> 'sectionTitle')) LIKE '%phần iii%' OR trim(lower(q ->> 'sectionTitle')) LIKE '%phần 3%')
      ) OR (
        (trim(lower(v_section_id)) LIKE '%phần ii%' OR trim(lower(v_section_id)) LIKE '%phần 2%')
        AND (trim(lower(q ->> 'sectionTitle')) LIKE '%phần ii%' OR trim(lower(q ->> 'sectionTitle')) LIKE '%phần 2%')
        AND NOT (trim(lower(q ->> 'sectionTitle')) LIKE '%phần iii%' OR trim(lower(q ->> 'sectionTitle')) LIKE '%phần 3%')
      ) OR (
        (trim(lower(v_section_id)) LIKE '%phần i%' OR trim(lower(v_section_id)) LIKE '%phần 1%')
        AND (trim(lower(q ->> 'sectionTitle')) LIKE '%phần i%' OR trim(lower(q ->> 'sectionTitle')) LIKE '%phần 1%')
        AND NOT (trim(lower(q ->> 'sectionTitle')) LIKE '%phần ii%' OR trim(lower(q ->> 'sectionTitle')) LIKE '%phần 2%')
        AND NOT (trim(lower(q ->> 'sectionTitle')) LIKE '%phần iii%' OR trim(lower(q ->> 'sectionTitle')) LIKE '%phần 3%')
      );
      ...
      IF v_config_type = 'THPT_QG' THEN
        ...
        IF trim(lower(v_section_id)) LIKE '%phần iii%' OR trim(lower(v_section_id)) LIKE '%phần 3%' THEN
          v_section_points := 3.0;
        ELSIF trim(lower(v_section_id)) LIKE '%phần ii%' OR trim(lower(v_section_id)) LIKE '%phần 2%' THEN
          v_section_points := 4.0;
        ELSIF trim(lower(v_section_id)) LIKE '%phần i%' OR trim(lower(v_section_id)) LIKE '%phần 1%' THEN
          v_section_points := 3.0;
        ELSE
      ...
      ELSE
        ...
        WHERE trim(lower(sec ->> 'section_id')) = trim(lower(v_section_id))
           OR (trim(lower(v_section_id)) LIKE '%phần iii%' OR trim(lower(v_section_id)) LIKE '%phần 3%' AND trim(lower(sec ->> 'section_id')) LIKE '%phần iii%' OR trim(lower(sec ->> 'section_id')) LIKE '%phần 3%')
           OR (trim(lower(v_section_id)) LIKE '%phần ii%' OR trim(lower(v_section_id)) LIKE '%phần 2%' AND trim(lower(sec ->> 'section_id')) LIKE '%phần ii%' OR trim(lower(sec ->> 'section_id')) LIKE '%phần 2%' AND NOT (trim(lower(sec ->> 'section_id')) LIKE '%phần iii%' OR trim(lower(sec ->> 'section_id')) LIKE '%phần 3%'))
           OR (trim(lower(v_section_id)) LIKE '%phần i%' OR trim(lower(v_section_id)) LIKE '%phần 1%' AND trim(lower(sec ->> 'section_id')) LIKE '%phần i%' OR trim(lower(sec ->> 'section_id')) LIKE '%phần 1%' AND NOT (trim(lower(sec ->> 'section_id')) LIKE '%phần ii%' OR trim(lower(sec ->> 'section_id')) LIKE '%phần 2%') AND NOT (trim(lower(sec ->> 'section_id')) LIKE '%phần iii%' OR trim(lower(sec ->> 'section_id')) LIKE '%phần 3%'));
      END IF;
...
-- 2. Tự động chấm lại toàn bộ các bài thi đã nộp trong quá khứ để cập nhật điểm số chuẩn xác
...
-- 3. Tạo hàm kiểm tra chấm điểm an toàn (để ta gọi bằng RPC từ server)
CREATE OR REPLACE FUNCTION public.temp_inspect_grading()
RETURNS jsonb AS $$
DECLARE
  v_res jsonb;
BEGIN
  SELECT jsonb_agg(jsonb_build_object(
    ...
    'score', ea.score,
    'answers', ea.answers,
    'recalculated', (SELECT to_jsonb(cs) FROM public.calculate_score(ea.quiz_id, ea.answers) cs)
  )) INTO v_res
  FROM public.exam_attempts ea
  JOIN public.quizzes q ON q.id = ea.quiz_id
  WHERE ea.status = 'submitted'
