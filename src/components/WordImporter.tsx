...
function WordImporter({ onQuestionsParsed }: WordImporterProps) {
    ...
    const [activeTab, setActiveTab] = useState<"paste" | "preview" | "raw">(
        "paste",
    );
    const [expandedHtmlQuestions, setExpandedHtmlQuestions] = useState<
        ...
        const handleKeyDown = (e: KeyboardEvent) => {
            ...
            if (e.key === "ArrowLeft") {
                ...
                setCurrentQuestionIdx((prev) =>
                    Math.min(parsedQuestions.length - 1, prev + 1),
                );
                e.preventDefault();
            }
        };
...
Lời giải: Vận tốc v(3) = 2*3 + 18 = 24.`;
     ...
     */
    const extractEquationNativeStream = (
        data: Uint8Array,
    ): Uint8Array | null => {
        ...
        try {
            ...
            while (curr !== 0xfffffffe && curr < fat.length) {
                ...
                if (offset + sectorSize <= data.length) {
                    ...
                    nextDir.set(dirData);
                    nextDir.set(
                        data.subarray(offset, offset + sectorSize),
                        dirData.length,
                    );
                    dirData = nextDir;
                }
                ...
                if (nameLen > 2) {
                    ...
                    for (let k = 0; k < nameLen - 2; k += 2) {
                        name += String.fromCharCode(
                            dirView.getUint16(offset + k, true),
                        );
                    }
            ...
            while (curr !== 0xfffffffe && curr < fat.length) {
                const offset = (curr + 1) * sectorSize;
                if (offset + sectorSize <= data.length) {
                    const nextMini = new Uint8Array(
                        miniFatData.length + sectorSize,
                    );
            ...
            if (miniFatData.length > 0) {
                const mfView = new DataView(
                    miniFatData.buffer,
                    miniFatData.byteOffset,
                    miniFatData.byteLength,
                );
            ...
            while (curr !== 0xfffffffe && curr < fat.length) {
                const offset = (curr + 1) * sectorSize;
                if (offset + sectorSize <= data.length) {
                    const nextStream = new Uint8Array(
                        miniStream.length + sectorSize,
                    );
            ...
            if (eqSize < miniCutoff) {
                ...
                while (
                    ...
                    if (offset + toRead <= miniStream.length) {
                        eqData.set(
                            miniStream.subarray(offset, offset + toRead),
                            bytesRead,
                        );
                        bytesRead += toRead;
                    }
                ...
                while (
                    ...
                    if (offset + toRead <= data.length) {
                        eqData.set(
                            data.subarray(offset, offset + toRead),
                            bytesRead,
                        );
                        bytesRead += toRead;
                    }
    ...
    const parseMtefBuffer = (rawBuf: Uint8Array): string => {
        ...
        while (i < buf.length - 1) {
                        ...
                        if (charCode > 0) {
                            if (isBracket && propertyCharsToSkip > 0) {
                                ...
                                const isIntegChar =
                                    ...
                                    charCode === 8749 ||
                                    charCode === 8750;
                                const isInsideIntegral = tmplStack.some(
                                    (t) => t.selector === 15,
                                );
                                if (isIntegChar && isInsideIntegral) {
                                    // skip duplicate integral symbol character
                                } else if (
                                    mathTypeSymbolMap[charCode] !== undefined
                                ) {
            ...
            if (tag === 8) {
                ...
                // COLOR selects color index
                idx += 1;
            } else if (tag === 16) {
                // COLOR_DEF defines color name string
                if (idx < buf.length) {
                    const opts = buf[idx];
                    idx += 1;
                    if (opts & 0x01) {
                ...
                node.childNodes.forEach((child) => {
                    ...
                    if (name === "e") {
                        ...
                        for (let j = 0; j < child.childNodes.length; j++) {
                            ...
                            const dPrChildName = dPrChild.nodeName.replace(
                                /^m:/,
                                "",
                            );
                            if (
                                ...
                                open =
                                    (dPrChild as Element).getAttribute(
                                        "m:val",
                                    ) ||
                                    (dPrChild as Element).getAttribute("val") ||
                                    "(";
                            } else if (
                                dPrChildName === "endCh" ||
                                dPrChildName === "end"
                ...
                node.childNodes.forEach((child) => {
                    ...
                    if (name === "naryPr") {
                        for (let j = 0; j < child.childNodes.length; j++) {
                            ...
                            const naryChildName = naryChild.nodeName.replace(
                                /^m:/,
                                "",
                            );
                            if (naryChildName === "chr") {
                                const val =
                                    (naryChild as Element).getAttribute(
                                        "m:val",
                                    ) ||
                                    (naryChild as Element).getAttribute("val");
                                if (val === "∫") op = "\\int";
                                else if (val === "∏") op = "\\prod";
                            }
                        }
     ...
     */
    const preProcessDocxBuffer = async (
        ...
        for (const [rId, targetPath] of Object.entries(rIdToTarget)) {
            const oleFile =
                zip.file(`word/${targetPath}`) || zip.file(targetPath);
            if (oleFile) {
        ...
        docXml = docXml.replace(
            /<w:drawing>[\s\S]*?<a:blip[^>]+r:(?:embed|link)="([^"]+)"[\s\S]*?<\/w:drawing>/g,
            (match, rId) => {
                const eq = oleEquMap[rId];
                if (eq) {
                    const escaped = escapeXml(eq);
        ...
        try {
            ...
            const oMathElements = xmlDoc.getElementsByTagNameNS(
                "http://schemas.openxmlformats.org/officeDocument/2006/math",
                "oMath",
            );
            const elementsList =
                oMathElements.length > 0
            ...
            elementsList.forEach((el) => {
                ...
                if (latex) {
                    const wR = xmlDoc.createElementNS(
                        "http://schemas.openxmlformats.org/wordprocessingml/2006/main",
                        "w:r",
                    );
                    const wT = xmlDoc.createElementNS(
                        "http://schemas.openxmlformats.org/wordprocessingml/2006/main",
                        "w:t",
                    );
            ...
            console.error(
                "Error parsing OMML math tags, falling back to regex mapping:",
                ommlErr,
            );
            docXml = docXml
                .replace(/<m:t([^>]*)>/g, "<w:t$1>")
        ...
        try {
            ...
            const pushCurrentQuestion = () => {
                if (currentQuestion && currentQuestion.text) {
                    currentQuestion.explanation = explanationHtmls
                        .join("")
                        .trim();
                    ...
                    if (
                        ...
                        currentQuestion.options = currentQuestion.options.map(
                            (opt) => {
                                ...
                                clean = clean.replace(
                                    /^(?:<(?:strong|b|span|p|em|i|u)[^>]*>)*\s*[A-D\(\)a-d][\.\:\-\)]\s*(?:<\/(?:strong|b|span|p|em|i|u)>)*\s*/i,
                                    "",
                                );
                    ...
                    if (
                        ...
                        const expText = currentQuestion.explanation.replace(
                            /<[^>]+>/g,
                            "",
                        );
                        const match = expText.match(
                            /(?:chọn|đáp án)\s*([A-D])/i,
                        );
                        if (match) {
                                ...
                                if (match2) {
                                    ...
                                    if (idx !== -1) {
                                        const slice = expText
                                            .toLowerCase()
                                            .slice(idx, idx + 100);
                                        if (slice.includes("đúng"))
                                            correctAnswers.push(true);
                                        else if (slice.includes("sai"))
            ...
            for (let i = 0; i < children.length; i++) {
                ...
                const hasOptions =
                    /^[A-D][\.\:\-\)]\s*/i.test(text) ||
                    /<strong>\s*[A-D][\.\:\-\)]\s*<\/strong>/i.test(
                        htmlContent,
                    ) ||
                    /A[\.\:\-\)]\s*.*B[\.\:\-\)]\s*.*C[\.\:\-\)]\s*.*D/i.test(
                        text,
                    );

                if (hasOptions && currentType === "single_choice") {
                    const optMatches = htmlContent.split(
                        /(?=<p>)?(?=<strong>\s*[A-D][\.\:\-\)])|(?<=\s|^|>)(?=[A-D][\.\:\-\)]\s*)/gi,
                    );
            ...
            if (questions.length === 0) {
                setError(
                    "Không tìm thấy cấu trúc câu hỏi hợp lệ. Vui lòng kiểm tra lại file.",
                );
                return;
            }
        ...
        try {
            if (file.name.endsWith(".docx") || file.name.endsWith(".doc")) {
                ...
                const htmlResult = await mammoth.convertToHtml(
                    { arrayBuffer: preProcessed },
                    options,
                );
                const html = htmlResult.value.replace(
                    /<img src=""\s*\/?>/g,
                    "",
                );
            ...
            setError(
                `Lỗi đọc file Word: ${err.message}. Vui lòng thử dán trực tiếp văn bản vào ô bên dưới.`,
            );
        } finally {
                    ...
                    <div className="border-2 border-dashed border-slate-200 hover:border-brand-400 rounded-xl p-6 text-center bg-slate-50/50 transition-colors">
                        ...
                        >
                            ...
                            <span className="text-xs font-bold text-slate-700">
                                {loadingFile
                                    ? "Đang trích xuất câu hỏi, hình ảnh & bảng biểu..."
                                    : "Tải lên file Word (.docx, .txt)"}
                            </span>
                            <span className="text-[11px] text-slate-400">
                                hoặc dán nội dung văn bản bên dưới
                            </span>
                        </label>
                    </div>
                        ...
                        <div className="bg-white border border-gray-100 rounded-xl p-6 sm:p-8 shadow-sm space-y-6 flex flex-col justify-between">
                            ...
                            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 border-b border-gray-100 pb-5">
                                ...
                                <div className="flex items-center gap-2 px-3.5 py-2 rounded-lg border bg-brand-50 border-brand-200 text-brand-700 text-xs font-bold self-start sm:self-auto">
                                    ...
                                    <span>
                                        Tổng số: {parsedQuestions.length} câu
                                    </span>
                                </div>
                            </div>
                                ...
                                <div className="space-y-1.5">
                                    ...
                                    <div className="w-full bg-slate-100 rounded-full h-1.5 overflow-hidden">
                                        <div
                                            ...
                                            style={{
                                                width: `${(configuredAnswersCount / parsedQuestions.length) * 100}%`,
                                            }}
                                        />
                                    </div>
                                </div>
            ...
            {activeTab === "raw" && (
                <div className="space-y-4">
                    <div className="p-3.5 bg-brand-50 border border-brand-200 rounded-xl text-xs text-brand-900 font-semibold flex items-center gap-2">
                        ...
                        <span>
                            Trình gỡ lỗi: Xem dữ liệu HTML/LaTeX gốc do
                            Mammoth.js trích xuất sau khi xử lý công thức.
                        </span>
                    </div>
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                        ...
                        <div className="space-y-2">
                            <label className="text-xs font-bold text-slate-700">
                                Trực quan hóa Giao diện Rendered (Có KaTeX):
                            </label>
                            <div
