...
const parseBugDescription = (desc: string) => {
    ...
    const match = trimmed.match(/^Tiêu đề:\s*([^\n]+)(?:\n+Chi tiết:\s*([\s\S]*))?$/i);
    if (match) {
        return {
...
export default function AdminBugsTab({
    ...
    // Delete Modal
    const [deleteModalBug, setDeleteModalBug] = useState<BugReport | null>(null);
            ...
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 pb-4 border-b border-border-primary/60">
                <div>
                    ...
                    <p className="text-xs text-slate-400 dark:text-slate-500 mt-0.5 font-medium">
                        Xem, theo dõi và xử lý các phản hồi, báo cáo sự cố từ học sinh và người dùng.
                    </p>
                </div>
            ...
            <div className="flex flex-col sm:flex-row gap-3 items-center justify-between">
                ...
                <div className="flex items-center gap-3 w-full sm:w-auto self-end sm:self-center">
                    ...
                    >
                        ...
                        <option value="student">Tài khoản học sinh ({studentCount})</option>
                        <option value="guest">Khách vãng lai ({guestCount})</option>
                    </select>
                </div>
            </div>
                ...
                <table className="w-full text-left border-1 border-slate-100 dark:border-slate-800/60">
                    <thead>
                        <tr className="border-b border-border-primary/50 text-[10px] font-bold text-slate-400 uppercase bg-slate-50/30 dark:bg-slate-800/20">
                            <th className="py-2.5 px-4 text-center w-12 whitespace-nowrap">STT</th>
                            <th className="py-2.5 px-4 w-52 whitespace-nowrap">Người gửi</th>
                            <th className="py-2.5 px-4 w-44 whitespace-nowrap">Thời gian tạo</th>
                            <th className="py-2.5 px-4 w-60 whitespace-nowrap">Tiêu đề</th>
                            <th className="py-2.5 px-4">Nội dung chi tiết</th>
                            <th className="py-2.5 px-4 text-right whitespace-nowrap w-24">Thao tác</th>
                        </tr>
                    </thead>
                        ...
                        {loading ? (
                            <tr>
                                <td colSpan={6} className="py-10 text-center text-slate-400">
                                    ...
                                    <span className="text-xs font-semibold">Đang tải danh sách báo cáo lỗi...</span>
                                </td>
                            </tr>
                            ...
                            paginatedReports.map((report, index) => {
                                ...
                                const isGoogle = isUserGoogleAccount(report as any);
                                const { title, details } = parseBugDescription(report.description);
                                ...
                                return (
                                    ...
                                    >
                                                ...
                                                {report.userId && (
                                                    <div className="flex items-center gap-1.5 text-[11px] text-slate-400">
                                                        <button
                                                            ...
                                                            onClick={() => {
                                                                if (report.userId) {
                                                                    onSelectUserForStats?.(report.userId);
                                                                }
                                                            }}
                                                        ...
                                                        {isGoogle && (
                                                            <span
                                                                ...
                                                                title={report.email ? `Google: ${report.email}` : "Tài khoản Google"}
                                                            >
                                        ...
                                        <td className="py-3 px-4 text-right whitespace-nowrap">
                                            <button
                                                type="button"
                                                onClick={() => setDeleteModalBug(report)}
            ...
            {totalPages > 1 && (
                <div className="flex items-center justify-between pt-2">
                    <span className="text-[11px] text-slate-400">
                        ...
                        {Math.min(currentPage * pageSize, filteredReports.length)} / {filteredReports.length} báo cáo
                    </span>
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40 backdrop-blur-xs animate-in fade-in duration-150">
                    <div className="bg-bg-card rounded-2xl border border-border-primary shadow-xl max-w-sm w-full p-6 space-y-4">
                        ...
                        <p className="text-xs text-slate-500 dark:text-slate-400 leading-relaxed">
                            Bạn có chắc chắn muốn xóa bản ghi báo cáo lỗi này của{" "}
